...
const article_id = elements['main_form'].getAttribute('data-article-id')
...
const upload_max_filesize = elements['article_attachments'].getAttribute('data-umf')
let max_file_uploads = parseInt(elements['article_attachments'].getAttribute('data-mfu'), 10)
if (max_file_uploads < 1 || max_file_uploads > 50) {
    // modify these limitations if required
    max_file_uploads = 20
}
let upload_max_filesize_bytes = parseInt(elements['article_attachments'].getAttribute('data-umfb'), 10)
...
const delete_text = ['Delete article', 'Please confirm deletion', 'DELETE - Final Confirmation']
...
function controlledDelete() {
    ...
    if (delete_stage_index >= count_delete_stages) {
        ...
        elements['delete_article_btn'].textContent = delete_text[delete_stage_index]
    }
}
...
idbRead('article', ['article_id', 'article_content'])
    .then((article_data) => {
        ...
        idbDelete('article', ['article_id', 'article_content'])
            .catch((reason) => {
                console.error('Failed to remove previous article_content: ' + reason)
            })

        try {
            // ID verification is critical!
            if (article_data.article_id === article_id &&
                typeof article_data.article_content === 'string') {
                backbone['article_content'] = {
                    // decode content first
                    content: atob_plus(article_data.article_content),
                    convert: false,
    ...
    .finally(() => {
        if (!Object.prototype.hasOwnProperty.call(backbone, 'article_content')) {
            const article_content_original_content = elements['article_content_original'].textContent.trim()
            backbone['article_content'] = {
                content: article_content_original_content
...
function controlledSubmit() {
    ...
    if (elements['front_form'].reportValidity()) {
        ...
        idbWrite('article', {
            ...
            'article_content': btoa_plus(elements['article_content_front'].value)
        })
